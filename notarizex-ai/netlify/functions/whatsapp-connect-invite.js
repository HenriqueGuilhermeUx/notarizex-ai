...
exports.handler=async(event)=>{
  ...
  try{
    ...
    if(action==='status'){
      const result=await callConfig(bot,'connect_status');const connected=result.config&&result.config.connected===true;if(connected&&!invite.completed_at)await touch(invite,{completed_at:new Date().toISOString()});
      return reply(200,{success:true,companyName:bot.company_name,setupStatus:result.setupStatus||null,setupError:result.setupError||null,config:sanitizeConfig(result.config),completed:connected});
    }
