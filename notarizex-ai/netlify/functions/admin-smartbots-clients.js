...
const headers={'Content-Type':'application/json','Cache-Control':'no-store','Access-Control-Allow-Origin':'*','Access-Control-Allow-Headers':'Content-Type, X-Admin-Key','Access-Control-Allow-Methods':'OPTIONS, POST'};
...
function adminKey(event,body){return clean((event.headers&&(event.headers['x-admin-key']||event.headers['X-Admin-Key']))||body.adminToken)}
function authorized(event,body){const expected=process.env.SMARTBOTS_ADMIN_API_KEY||process.env.SMARTBOTS_ADMIN_TOKEN||process.env.ADMIN_TOKEN;return Boolean(expected&&safeEqual(adminKey(event,body),expected))}
...
async function listClients(){
  const [bots,wa,profiles,invites]=await Promise.all([
    rows(await db('website_bots?select=bot_id,company_name,owner_name,owner_email,owner_whatsapp,status,plan,website,created_at&order=created_at.desc&limit=200'),'bots'),
  ...
  return bots.map(bot=>({
    ...bot,
    profile:profileMap.get(bot.bot_id)||null,
    whatsapp:waMap.get(bot.bot_id)||null,
    latestInvite:inviteMap.get(bot.bot_id)||null
  }));
}
...
exports.handler=async(event)=>{
  ...
  if(!authorized(event,body))return reply(401,{success:false,error:'Não autorizado.'});
  try{
