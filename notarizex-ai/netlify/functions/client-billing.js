...
function activePeriod(row) {
  return Boolean(row && row.status === 'active' && row.current_period_end && new Date(row.current_period_end).getTime() > Date.now());
}
...
async function saveSubscription(bot, existing, plan, correlationID, charge, raw) {
  const patch = {
    ...
    woovi_charge_id: charge.globalID || charge.chargeId || charge.id || charge.identifier || '',
    woovi_payment_link: charge.paymentLinkUrl || charge.paymentLink || '',
...
async function createCharge(bot, existing) {
  ...
  const payload = {
    ...
    customer: { name: bot.company_name || 'Cliente SmartBots', email: bot.email || bot.owner_email || '' }
  };
  ...
  if (!response.ok) {
    console.error('client-billing woovi', response.status, JSON.stringify(raw).slice(0,500));
  ...
  try {
    ...
    if (body.action === 'status') {
      ...
      return reply(200,{
        ...
        offer:{planCode:offered.code,planName:offered.name,amountCents:offered.amountCents,regularAmountCents:PLANS.completo.amountCents,founderSlotsRemaining:slots.available,founderLimit:slots.limit,trialEndsAt:bot.trial_ends_at || null,billingStatus:bot.billing_status || null}
      });
    }
