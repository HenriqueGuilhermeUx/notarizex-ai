...
async function loadClients(){
  ...
  list.innerHTML=(j.items||[]).map(function(x){var link='/admin-assinaturas.html?botId='+encodeURIComponent(x.bot_id||'');return '<div class="bg-slate-900 border border-white/10 rounded-xl p-4 mb-3"><b>'+String(x.company_name||x.bot_id)+'</b><p>'+String(x.plan||'')+' - '+String(x.status||'')+'</p><p>'+String(x.customer_email||'')+'</p><a href="'+link+'" class="text-green-400">Editar</a></div>'}).join('')||'Nenhum cliente';
}
